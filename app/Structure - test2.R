...
library(ggmap)
library(leaflet)
college<-read.csv(file="~/GitHub/Spr2017-proj2-grp4/data/College2014_15_new.csv", stringsAsFactors = FALSE,na.strings = "NULL")
...
server = function(input, output){
  ...
  output$map = renderLeaflet({
    ...
    addCircleMarkers(lng = map$lon, lat = map$lat, fillColor=cPal(map$degree),clusterOptions = markerClusterOptions(), stroke=FALSE, fillOpacity=0.8, popup=map$conm) %>%
